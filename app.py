...
def exe():
    ...
    if request.method == "POST":
        if request.form['submit'] == 'runFile':
            ...
            Missile.MissileModelling(x0, y0, v0, tet0, outT, outVx, outVy, outX, outY, ctypes.byref(size))
            
            # Convert the C arrays to Python lists
            outT_list = [round(outT[i],2) for i in range(size.value)]
            ...
            flat_list = list(zip(outT_list, outVx_list, outVy_list, outX_list, outY_list))
        ...
        return render_template("assemblies.html", pic=current_dir + "/flight.jpg")
